...
class Expression private constructor() {
    ...
    fun asExpressionHolder(): ExpressionHolder {
        val argsList = mutableListOf<Table>()
        val rootExpr = traverseExprRec(this, argsList)

        return ExpressionHolder(argsList.map { it.asDto() }, rootExpr)
    }

    // traverse the "tree" of expression
    private fun traverseExprRec(
        expr: Expression,
        argsList: MutableList<Table>,
    ): ExpressionDto {
        ...
        val leftTableDto = expr.leftTable?.let {
            if (it in argsList) {
                argsList.indexOf(it)
            } else {
                argsList.add(it)
        ...
        val rightTableDto = expr.rightTable?.let {
            if (it in argsList) {
                argsList.indexOf(it)
            } else {
                argsList.add(it)
